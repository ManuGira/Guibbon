...
# Guibbon2 — Architecture & Implementation Reference
...
├── apps/
│   ├── simple_image_viewer.py   # Pattern 1: show/wait (Phase 3)
│   ├── controller_app.py        # Pattern 2: ControllerAppBase (Phase 3)
│   └── image_app.py             # Pattern 3: InteractiveImageAppBase (Phase 3)
...
- `Descriptor` (ABC): Base class with `default`, `is_visible`, `triggered_callbacks`, `on_widget_change()`, `get_triggered_descriptors()`
- `BuildableDescriptor`: Descriptor subclass with optional `widget_class` for custom widgets
- `SliderDescriptor` (in `controller/`): values range, on_drag/on_release flags, callback tracking
- `RadioDescriptor` (in `controller/`): options list, on_change flag, callback tracking
...
| Descriptor base | ABC with on_widget_change + get_triggered_descriptors | Clear contracts, component-agnostic |
| Descriptor organization | Component modules (controller/, image_viewer/) | Zero framework coupling, pluggable |
| Custom widgets? | BuildableDescriptor + widget_class | User writes plain Tkinter; zero coupling |
...
**Module 1 (params.py):**
- `src/guibbon/core/params.py` — @guibbon.params decorator, tracked types, GetPath
- `tests/test_params.py` — 30 passing tests
- `examples/demo_params.py` — decorator demo

**Module 2 (descriptor.py + controller/):**
- `src/guibbon/core/descriptor.py` — Descriptor ABC, BuildableDescriptor
- `src/guibbon/controller/slider.py` — SliderDescriptor
...
- `.github/instructions/core-implementation.instructions.md` — Phase 1 coding patterns
